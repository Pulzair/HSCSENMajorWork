-- Battley database schema
-- Notes:
--  * SQLite has no BOOLEAN type, so booleans are stored as INTEGER 0/1 with a CHECK.
--  * SQLite has no dedicated DATETIME type; timestamps are TEXT stored in
--    'YYYY-MM-DD HH:MM:SS' format via datetime('now').
...
CREATE TABLE Orders (
    ...
    -- unit_id and the territory columns are nullable because not every order
    -- type uses them (e.g. a 'build' order need not act on an existing unit).
