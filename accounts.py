...
# the re compile compiles regex for string validation
EMAIL_RE = re.compile(r"^[^@\s]+@[^@\s]+\.[^@\s]+$")
MIN_PASSWORD = 8
MAX_BIO = 160
AUTH_LIMIT = "10 per minute" # saved as the error message
...
# ensure user is logged in before an app request is made
@bp.before_app_request
def load_logged_in_user():
	user_id = session.get("user_id")
	# check user_id in users and the execute database SQL code selecting all user information (read the SQL pseudocode)
...
# if not login in and login_required (as it is for like 90% of usecases)
def login_required(view):
	@functools.wraps(view)
	def wrapped(**kwargs): # love some kwargs, infinite amount of keywords
		if g.user is None:
...
def admin_required(view):
	...
	def wrapped(**kwargs):
		...
		if not g.user["is_admin"]:
			...
			# ABORT if tries admin command without admin perms
			abort(403)
		return view(**kwargs)
	return wrapped
...
def register():
	if g.user:
		return redirect(url_for("dashboard")) # return to dashboard once logged in
	...
	if request.method == "POST":
		...
		errors = [] # validat user information
		...
		if not errors: # if no problems in validation
			db = get_db()
			try:
				...
				db.commit() # update username
			except sqlite3.IntegrityError:
				errors.append("That username or email is already taken.") # error
			else:
				flash("Account created. Log in to start playing.", "success") # success
				return redirect(url_for("accounts.login"))

		for error in errors:
			flash(error, "error") # return errors

	return render_template("account.html", mode="register") # link to account.html page

# login route
@bp.route("/login", methods=("GET", "POST"))
@limiter.limit(AUTH_LIMIT, methods=["POST"]) # rate limiting
def login():
	if g.user:
		return redirect(url_for("dashboard")) # if logged in return to dashboard
	...
	if request.method == "POST":
		...
		if user is None or not bcrypt.check_password_hash(user["password_hash"], request.form.get("password", "")):
			flash("Incorrect email or password.", "error") # error message
		else:
			session.clear()
...
def logout():
	session.clear()
	flash("You have been logged out.", "success") # debugging message
...
def profile():
	if request.method == "POST":
		...
		new_password = request.form.get("new_password", "") # request all the information

		errors = []
		if not USERNAME_RE.match(username): # bunch of errors
			errors.append("Usernames are 3 to 32 letters, numbers or underscores.")
		if not EMAIL_RE.match(email) or len(email) > 254:
			errors.append("That does not look like an email address.")
		if len(bio) > MAX_BIO:
		...
		changed = username != g.user["username"] or email != g.user["email"] # if change username on profile page
		...
		if errors:
			for error in errors:
				flash(error, "error")
			return render_template("account.html", mode="profile") # render profile paper
		...
		try: # try update
			...
			db.commit()
		except sqlite3.IntegrityError: # except already exists then give error (i.e. can't overrider or log into others account)
			db.rollback()
			flash("That username or email is already taken.", "error")
			return render_template("account.html", mode="profile")

		flash("Profile updated.", "success") # redirect
...
@admin_required
def admin():
	db = get_db() # check all active games and users
...
def summary(db, user_id): 
	...
	credit = won + drawn * 0.5 # draws count as half for both players
	...
	# reputation maintained across games to fufill diplomacy requirements
...
# pull the last few finished games with everyones final standings for the dashboard
...
# calculate leaderboard points for ranks
...
# generate standings in leaderboard based off the rank points
def standings(db):
	table = []
